...
def mesh(shape_y, shape_x, height, m):
    ...
    l, l2, coor = 20, int(shape_y//2 - 4*height), 260
    l, l2, coor = l*m, l2, coor*m
    mask[:l2,  coor :coor +l] = wall(l, l2)
    mask[shape_y-l2:,  coor :coor +l] = np.flipud(wall(l, l2)) + 2
...
for i in range(1,time):
    ...
    for j in range(1, size): 
        ...
        if is_in[j]:
            ...
            if vel!=0 and vx_f[j] == 1: mas.append(vel)
...
params = {'sh': shape_x,'len': size, 'time': time,
            'height':height,'bias':bias,'thresh':thresh,
