...
def load(filename, title, sr, sigma):
    ...
    s = 1*np.round(np.load('data/' + filename))
...
def characteristics(sl, lab):

    sq = np.trapz(sl)
    arg_pol = np.argmin(np.abs(sl - np.max(sl)/2))
    arg_full = np.argmin(np.abs(sl - np.max(sl)))
    semi_wid = np.abs(arg_full - arg_pol)
    print(semi_wid, sq)
    new_row = [lab, semi_wid, sq]
...
name = 'now'
lab = name
slice = load(name +'.npy', lab, 250+150, 0)[:, 250+100]
window_size = 15
